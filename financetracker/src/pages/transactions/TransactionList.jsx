...
const TransactionList = () => {
  ...
  const handleReload = async () => {
    try {
      ...
      showNotification('Transactions reloaded', 'info')
    } catch {
      // error is handled in context state
    }
  }

  const getIconClasses = (tx) => {
    if (tx.type === 'income') {
      return 'bg-matcha/20 dark:bg-green-900/30 text-matcha dark:text-green-400'
    }
    if (tx.category === 'Housing') {
      return 'bg-honey-drizzle/20 dark:bg-orange-900/30 text-coffee-grounds/70 dark:text-gray-400'
    }
    if (tx.category === 'Food') {
      return 'bg-lemonade/20 dark:bg-yellow-900/30 text-coffee-grounds dark:text-gray-100'
    }
    return 'bg-honey-drizzle/20 dark:bg-orange-900/30 text-coffee-grounds/70 dark:text-gray-400'
  }

  const getAmountClasses = (tx) => {
    if (tx.type === 'income') {
      return 'text-matcha dark:text-green-400 font-semibold'
    }
    return 'text-honey-drizzle dark:text-orange-400 font-semibold'
  }
    ...
    <div className={`bg-gradient-to-br from-sea-blue/5 to-matcha/5 dark:from-slate-900 dark:to-slate-800 text-coffee-grounds dark:text-gray-100 font-sans min-h-screen ${isDarkMode ? 'dark' : ''}`}>
      ...
      <section className="bg-white dark:bg-slate-800 mt-6 mx-4 md:mx-8 p-6 rounded-xl shadow-lg border border-honey-drizzle/50 dark:border-slate-700">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="relative">
              <svg
                ...
                className="absolute left-3 top-2.5 w-4 h-4 text-coffee-grounds/50 dark:text-gray-500"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                aria-hidden="true"
        ...
        <div className="mt-4 flex justify-end gap-4">
          {transactionsLoading && (
            <span className="text-xs text-coffee-grounds/60 dark:text-gray-400">
          ...
          {transactionsError && (
            <button
              ...
              className="text-red-500 dark:text-red-400 text-xs underline"
            >
              Retry loading
            </button>
          )}
          <button
            ...
            className="text-sea-blue dark:text-blue-400 hover:underline text-sm font-medium"
          >
            Reset Filters
          </button>
        </div>
      ...
      <section className="mx-4 md:mx-8 mt-8 mb-8">
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="flex gap-3 text-sm text-coffee-grounds/70 dark:text-gray-400">
            <span className="hover:text-sea-blue dark:hover:text-blue-400 cursor-not-allowed select-none">
              Export
            </span>
            <span>|</span>
            <span className="hover:text-sea-blue dark:hover:text-blue-400 cursor-not-allowed select-none">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg divide-y divide-honey-drizzle/50 dark:divide-slate-700">
          {filtered.length === 0 && !transactionsLoading && (
            <div className="p-4 text-sm text-coffee-grounds/70 dark:text-gray-400">
          ...
          {filtered.map((tx) => (
            <div
              key={tx.id}
              className="flex justify-between items-center p-4 hover:bg-sea-blue/5 dark:hover:bg-slate-700"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-coffee-grounds/70 dark:text-gray-400 text-sm">
                    {formatDate(tx.date)} · {tx.category}
                  </p>
                </div>
              </div>
              <div className="text-right">
                <p className={getAmountClasses(tx)}>
                  {tx.type === 'expense' ? '- ' : '+ '}
                  {formatCurrency(tx.amount)}
                </p>
                <Link
                  to={`/transactions/${tx.id}`}
                  className="text-sea-blue dark:text-blue-400 text-sm hover:underline"
                >
                  View Details
                </Link>
              </div>
